...
let rec check_redecl decs tl vl =
     ...
     | VarDecValue (_,s,_)::rest -> if List.mem s vl then raise (SymErr s)
                               else check_redecl rest tl (s::vl)
...
let rec type_dec ast (nest,addr) tenv env =
   match ast with
     ...
     | VarDecValue (t, s, e) ->
          (* 型チェック: 宣言された型と初期値の型が一致するか確認 *)
          let ty = create_ty t tenv in
               if ty != (type_exp e env) then raise (TypeErr "type error 10")
               else (tenv, update s (VarEntry {ty; offset=addr-8; level=nest}) env, addr-8)
...
and type_stmt ast env =
       match ast with
          ...
          (* | For (v, el, er, _) ->
               if (type_var v env) != (type_exp el env) then raise (TypeErr "type error 11");
               if (type_var v env) != (type_exp er env) then raise (TypeErr "type error 12") *)
          | Incr v ->
               if (type_var v env) != INT then
                    raise (TypeErr "incriment requires Var")
          | NilStmt -> ()
...
and type_exp ast env =
        match ast with
          ...
          | IntExp i -> INT
          | CallFunc ("++", [left]) ->
               (check_int (type_exp left env); INT)
          | CallFunc ("+", [left; right]) ->
